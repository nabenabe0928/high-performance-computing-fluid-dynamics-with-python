...
\begin{algorithm}[tb]
  ...
  \begin{algorithmic}[1]
    ...
    \Comment{Eq~(\ref{discretized-density}), (\ref{discretized-velocity})}
    \EndFor
    \EndFunction
  \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
    ...
    \State{$f$[0][:,out\_indices]=$\feq_{\rm in}$[:,out\_indices]+($f$[-2][:,out\_indices]-$\feq$[-2][:,out\_indices])}
    \State{$f$[-1][:, in\_indices]=$\feq_{\rm out}$[:, in\_indices]+($f$[1][:, in\_indices] - $\feq$[1][:, in\_indices])}
    \EndIf
    \State{{\bf return} $f$}
    \EndFunction
  \end{algorithmic}
