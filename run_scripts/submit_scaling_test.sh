...
N_THREADS_LIST=(100 144 225 400)
# for S in 100 300 1000; do
for S in 100 1000; do
    for N_THREADS in ${N_THREADS_LIST[@]}; do
        NODES=$((${N_THREADS}/40+1))
        echo "Use ${N_THREADS} threads on ${NODES} nodes."
        echo sbatch --nodes=$NODES --ntasks-per-node=40 run_scripts/run_server.sh -N $N_THREADS -S $S
        sbatch --nodes=$NODES --ntasks-per-node=40 run_scripts/run_server.sh -N $N_THREADS -S $S
        echo ""
    done
done
